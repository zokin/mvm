...
// Custom frustum functon that also returns the end quad's vertices
std::vector<float> frustum2(DD_EXPLICIT_CONTEXT_ONLY(ContextHandle ctx, ) ddMat4x4_In matrix,
	ddVec3_In color, const float scale = 1.0f,
	const float fov_y = 64.f, const float focal = 600.f / 1.f,
	const float aspect = 320.f / 180.f, const float near_plane = 0.05f,
	const float far_plane = 0.1f,
	const int durationMillis = 0, const bool depthEnabled = false);
...
std::vector<float> frustum2(DD_EXPLICIT_CONTEXT_ONLY(ContextHandle ctx, ) ddMat4x4_In matrix,
	ddVec3_In color, const float scale,
	const float fov_y, const float focal,
	const float aspect, const float near_plane,
	...
	const int durationMillis, const bool depthEnabled)
{
	...
	float tangent = std::tanf(dd::degreesToRadians(fov_y / 2.0));
	float nearHeight = scale * near_plane * tangent;
	float nearWidth = nearHeight * aspect;
	float farHeight = scale * far_plane * tangent;
	float farWidth = farHeight * aspect;
	float planes[8][3];
	planes[0][0] = nearWidth;     planes[0][1] = nearHeight;    planes[0][2] = scale * near_plane;
	planes[1][0] = -nearWidth;    planes[1][1] = nearHeight;    planes[1][2] = scale * near_plane;
	planes[2][0] = -nearWidth;    planes[2][1] = -nearHeight;   planes[2][2] = scale * near_plane;
	planes[3][0] = nearWidth;     planes[3][1] = -nearHeight;   planes[3][2] = scale * near_plane;

	planes[4][0] = farWidth;     planes[4][1] = farHeight;    planes[4][2] = scale * far_plane;
	planes[5][0] = -farWidth;    planes[5][1] = farHeight;    planes[5][2] = scale * far_plane;
	planes[6][0] = -farWidth;    planes[6][1] = -farHeight;   planes[6][2] = scale * far_plane;
	planes[7][0] = farWidth;     planes[7][1] = -farHeight;   planes[7][2] = scale * far_plane;
	...
	for (int i = 0; i < arrayLength(planes); ++i)
	{
		wCoords[i] = matTransformPointXYZW2(points[i], planes[i], matrix);
	}

	std::vector<float> far_quad(4 * 3);
	far_quad[0 * 3 + 0] = points[4][0]; far_quad[0 * 3 + 1] = points[4][1]; far_quad[0 * 3 + 2] = points[4][2];
	far_quad[1 * 3 + 0] = points[5][0]; far_quad[1 * 3 + 1] = points[5][1]; far_quad[1 * 3 + 2] = points[5][2];
	far_quad[2 * 3 + 0] = points[6][0]; far_quad[2 * 3 + 1] = points[6][1]; far_quad[2 * 3 + 2] = points[6][2];
	far_quad[3 * 3 + 0] = points[7][0]; far_quad[3 * 3 + 1] = points[7][1]; far_quad[3 * 3 + 2] = points[7][2];
...
void structure(DD_EXPLICIT_CONTEXT_ONLY(ContextHandle ctx, ) ddVec3_In color)
{
	if (!isInitialized(DD_EXPLICIT_CONTEXT_ONLY(ctx)))
	{
		return;
	}
	static ddVec3 mid_bottom_box[8] =
	{
		{ 50.000397f / 1000.f, -0.000796f / 1000.f, 165.000549f / 1000.f }, // 0
		{ 50.000427f / 1000.f, -0.000583f / 1000.f, -394.999421f / 1000.f }, // 1
		{ 50.000427f / 1000.f, -410.000092f / 1000.f, -394.999604f / 1000.f }, // 2
		{ 50.000397f / 1000.f, -410.000305f / 1000.f, 165.000396f / 1000.f }, // 3

		{ -279.999908f / 1000.f, -0.000796f / 1000.f, 165.000762f / 1000.f }, // 4
		{ -279.999847f / 1000.f, -0.000583f / 1000.f, -394.99936f / 1000.f }, // 5
		{ -279.999847f / 1000.f, -410.000092f / 1000.f, -394.99936f / 1000.f }, // 6
		{ -279.999908f / 1000.f, -410.000305f / 1000.f, 165.000762f / 1000.f }, // 7				
	};

	static ddVec3 mid_top_box[8] =
	{
		{ -509.999573f / 1000.f, 410.000061f / 1000.f, -165.000549f / 1000.f }, // 0
		{ 50.000427f / 1000.f, 410.000061f / 1000.f, -165.000549f / 1000.f }, // 1
		{ 50.000427f / 1000.f, 0.000554f / 1000.f, -165.000549f / 1000.f }, // 2
		{ -509.999573f / 1000.f, 0.000554f / 1000.f, -165.000396f / 1000.f }, // 3

		{ -509.999573f / 1000.f, 410.000061f / 1000.f, 164.999451f / 1000.f }, // 4
		{ 50.000427f / 1000.f, 410.000061f / 1000.f, 164.999451f / 1000.f }, // 5
		{ 50.000427f / 1000.f, 0.000554f / 1000.f, 164.999451f / 1000.f }, // 6
		{ -509.999573f / 1000.f, 0.000554f / 1000.f, 164.999451f / 1000.f }, // 7				
	};

	static ddVec3 bottom_box[8] =
	{
		{ -279.999908f / 1000.f, -410.000305f / 1000.f, -164.999649f / 1000.f }, // 0		
		{ -279.999908f / 1000.f, -819.999817f / 1000.f, -164.999649f / 1000.f }, // 1
		{ -279.999847f / 1000.f, -819.999817f / 1000.f, 165.000762f / 1000.f }, // 2
		{ -279.999847f / 1000.f, -410.000092f / 1000.f, 165.000762f / 1000.f }, // 3
		
		{ 280.000031f / 1000.f, -410.000305f / 1000.f, -164.999649f / 1000.f }, // 4
		{ 280.000031f / 1000.f, -819.999817f / 1000.f, -164.999649f / 1000.f }, // 5
		{ 280.000031f / 1000.f, -819.999817f / 1000.f, 165.000762f / 1000.f }, // 6
		{ 280.000031f / 1000.f, -410.000092f / 1000.f, 165.000762f / 1000.f }, // 7
	};

	static ddVec3 top_box[8] =
	{
		{ 50.000427f / 1000.f, 819.999573f / 1000.f, 394.999421f / 1000.f }, // 0
		{ 50.000427f / 1000.f, 410.000092f / 1000.f, 394.999604f / 1000.f }, // 1
		{ 50.000397f / 1000.f, 410.000305f / 1000.f, -165.000396f / 1000.f }, // 2
		{ 50.000397f / 1000.f, 819.999573f / 1000.f, -165.000549f / 1000.f }, // 3

		{ -279.999908f / 1000.f, 819.999573f / 1000.f, 394.999421f / 1000.f }, // 4
		{ -279.999908f / 1000.f,  410.000092f / 1000.f, 394.999421f / 1000.f }, // 5
		{ -279.999908f / 1000.f,  410.000092f / 1000.f, -165.000396f / 1000.f }, // 6		
		{ -279.999847f / 1000.f, 819.999573f / 1000.f, -165.000396f / 1000.f }, // 7					
	};
	
	box(mid_bottom_box, color, 0, false);
	box(mid_top_box, color, 0, false);
	box(top_box, color, 0, false);
	box(bottom_box, color, 0, false);
}
